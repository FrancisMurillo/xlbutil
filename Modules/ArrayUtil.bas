...
'R Normal Array
Public Function Zip(ParamArray Arr() As Variant) As Variant
    Dim MinSize As Long, Arr_ As Variant, Size_ As Long
    MinSize = Size(Arr(0))
    For Each Arr_ In Arr
        If IsEmptyArray(Arr_) Then
            Zip = CreateEmptyArray()
            Exit Function
        Else
            Size_ = Size(Arr_)
    ...
    Tuple = CreateWithSize(UBound(Arr) + 1)
    
    For Index = 0 To UBound(ZArr)
        For TIndex = 0 To UBound(Arr)
            Tuple(TIndex) = Arr(TIndex)(LBound(Arr(TIndex)) + Index)
        Next
        ZArr(Index) = Tuple
    Next
